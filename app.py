...
app = dash.Dash(name="weather", server=server, url_base_pathname="/")
app.layout = html.Div(children=[
    dcc.Location(id='url', refresh=False),
    html.H1(children='Прогноз погоды'),

    html.Div(id='cities', children=[
        dcc.Input(id={'type': 'city', 'index' : '1'}, type='text', placeholder='Начальный город', value=""),
        dcc.Input(id={'type': 'city', 'index' : '2'}, type='text', placeholder='Конечный город', value=""),
]),
...
@callback(
    Output({'type': 'city', 'index': '1'}, 'value'),
    Output({'type': 'city', 'index': '2'}, 'value'),
    Output('submit-val', 'n_clicks'),
    Input('url', 'search')
)
def update_output_link(search):
    if not search:
        return '', ''
    ...
    city1, city2 = [x.strip().replace('_', ' ') for x in cities]

    return city1, city2, 1
